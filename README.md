...
- `vercel.json` の Cron が毎日1回（UTC 6:00 / JST 15:00ごろ）呼び出します。Cron の本数は2026年1月以降どのプランでも1プロジェクト100本までです。実行頻度は Hobby が1日1回・時間単位の誤差、Pro は分単位で指定できます。停止まで7日の猶予があるので、1日1回で十分な間隔です。
