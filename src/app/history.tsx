      ...
      Promise.all([isHistoryUserSignedIn(), getDrillHistory()])
        .then(([signedIn, entries]) => {
          ...
          entries.forEach((entry) => {
            ...
            row.sentences += entry.sentenceCount;
            if (!row.drills.includes(entry.drillId)) row.drills.push(entry.drillId);
            grouped.set(entry.date, row);
          });
          setRows(Array.from(grouped.values()).reverse());
        })
        .catch(() => {
          setIsSignedIn(false);
          setRows([]);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.note}>取り組んだ時間と例文数を日ごとに確認できます。</Text>
        {!isSignedIn ? (
          <View style={styles.empty}>
            <Text style={styles.emptyTitle}>ログインすると練習記録を残せます</Text>
          ...
          <View style={styles.empty}>
            ...
            <Text style={styles.note}>最初のドリルを1文読んでみましょう。</Text>
              ...
              <View key={row.date} style={styles.row}>
                <View>
                  ...
                  <Text style={styles.drills}>
                    {row.drills
                      .map((id) => DRILLS.find((d) => d.id === id)?.shortTitle)
                      .filter(Boolean)
                      .join('・')}
                  </Text>
                </View>
                <Text style={styles.value}>
                  {Math.max(1, Math.round(row.seconds / 60))}分・{row.sentences}文
                </Text>
              </View>
            ))}
          </View>
