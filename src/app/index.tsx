...
function HomeScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      let active = true;
      Promise.all([isHistoryUserSignedIn(), getDrillHistory(), getAssessmentHistory()])
        .then(([signedIn, drills, checks]) => {
          if (!active) return;
  ...
  const monthEntries = Object.entries(historyByDate).filter(([date]) =>
    date.startsWith(`${year}-${String(monthIndex + 1).padStart(2, '0')}`)
  );
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
          ...
          {daysUntilCheck === 0 && (
            <Link href="/check" asChild>
              <Pressable style={styles.checkButton}>
                <Text style={styles.checkButtonText}>発話チェックを始める</Text>
              </Pressable>
            </Link>
          )}
        </View>
        ...
        <View style={styles.sectionHeader}>
          ...
          <Link href="/drills" style={styles.textLink}>
            すべて見る
          </Link>
        </View>
          ...
          {recommended.map(
            (drill) =>
              drill && (
                <Pressable
                  key={drill.id}
                  style={[styles.recommendCard, { backgroundColor: drill.accent }]}
                  onPress={() => router.push({ pathname: '/drills', params: { drill: drill.id } })}>
                  <Text style={styles.recommendTag}>おすすめドリル</Text>
                  <Text style={styles.recommendTitle}>{drill.title}</Text>
                  <Text style={styles.recommendBody}>{drill.description}</Text>
                  <Text style={styles.recommendAction}>練習する →</Text>
                </Pressable>
              )
          )}
        </View>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>発話カレンダー</Text>
          {isSignedIn && (
            <Link href="/history" style={styles.textLink}>
              履歴を見る
            </Link>
          )}
        </View>
        <View style={styles.calendarCard}>
          ...
          {isSignedIn && (
            <Text style={styles.monthSummary}>
              今月 {Math.round(monthSeconds / 60)}分・{monthSentences}文・{monthEntries.length}
              日練習
            </Text>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  checkButtonText: { color: palette.greenDark, fontSize: 16, fontWeight: '800' },
  sectionHeader: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  textLink: { color: palette.green, fontSize: 15, fontWeight: '700' },
  ...
  recommendTag: { color: palette.greenDark, fontSize: 12, fontWeight: '800' },
  recommendTitle: { color: palette.ink, fontSize: 20, fontWeight: '800', marginTop: 5 },
  recommendBody: { color: palette.muted, fontSize: 15, lineHeight: 23, marginTop: 7 },
  recommendAction: {
  ...
  monthArrow: { color: palette.greenDark, fontSize: 30, paddingHorizontal: 8 },
