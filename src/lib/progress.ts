...
export async function getDrillHistory(): Promise<DrillHistoryEntry[]> {
  ...
  const { data, error } = await supabase
    .from('drill_sessions')
    .select('id, performed_at, drill_id, duration_seconds, sentence_count')
    .eq('user_id', userId)
    .order('performed_at', { ascending: true });
  if (error) throw error;
  return (data ?? []).map((row) => ({
    id: row.id,
    date: localDateKey(new Date(row.performed_at)),
...
export async function getAssessmentHistory(): Promise<AssessmentHistoryEntry[]> {
  ...
  const { data, error } = await supabase
    ...
    .select('id, performed_at, headline, recommended_drill_ids')
    .eq('user_id', userId)
    .order('performed_at', { ascending: true });
  if (error) throw error;
  return (data ?? []).map((row) => ({
    ...
    recommendedDrillIds: row.recommended_drill_ids ?? [],
  }));
}
...
export async function saveDrillHistory(entry: Omit<DrillHistoryEntry, 'id' | 'date'>) {
  ...
  const { error } = await supabase.from('drill_sessions').insert({
    ...
    sentence_count: entry.sentenceCount,
  });
  if (error) throw error;
  return true;
}
...
export async function saveAssessmentHistory(entry: Omit<AssessmentHistoryEntry, 'id' | 'date'>) {
  ...
  const { error } = await supabase.from('assessment_sessions').insert({
    ...
    recommended_drill_ids: entry.recommendedDrillIds,
  });
  if (error) throw error;
  return true;
}
